...
# Enhanced file utilities for the restructured system
def list_available_files(data_dir: str = None) -> Dict[str, Any]:
    """List all available data files in the input directory."""
    try:
        if data_dir is None:
            from config_manager import get_config
            config = get_config()
            data_dir = config.get('data.input_dir', './data/input')
        ...
        if csv_path.exists():
            files['csv_files'] = [str(f.relative_to(csv_path)) for f in csv_path.glob('*.csv')]
        
        # Find Markdown files
        md_path = data_path / 'markdown'
        if md_path.exists():
            files['markdown_files'] = [str(f.relative_to(md_path)) for f in md_path.rglob('*.md')]
        
        # Find other files
        for ext in ['*.json', '*.txt', '*.yaml', '*.yml']:
            files['other_files'].extend([str(f.relative_to(data_path)) for f in data_path.rglob(ext)])
        
        return tool_success("available_files", files)
        
    except Exception as e:
        return tool_error(f"Error listing files: {str(e)}")

def validate_file_for_import(file_path: str, file_type: str = "auto") -> Dict[str, Any]:
    """Validate that a file is suitable for import."""
    try:
        full_path = Path(get_neo4j_import_dir() or "") / file_path
        ...
        if full_path.stat().st_size > 10 * 1024 * 1024:  # 10MB
            validation_result["warnings"].append("Large file size may affect performance")
        
        return tool_success("validation_result", validation_result)
        
    except Exception as e:
        return tool_error(f"File validation error: {str(e)}")

def suggest_files_for_goal(goal_description: str, graph_type: str) -> Dict[str, Any]:
    """Suggest relevant files based on user goal."""
    try:
        available_files_result = list_available_files()
        if available_files_result['status'] != 'success':
            return available_files_result
        
        available_files = available_files_result['available_files']
        suggestions = {
            'primary_files': [],
            'secondary_files': [],
            'reasoning': []
        ...
        if any(keyword in goal_lower for keyword in ['product', 'catalog', 'inventory', 'furniture']):
            if 'products.csv' in available_files['csv_files']:
                suggestions['primary_files'].append('csv/products.csv')
                suggestions['reasoning'].append("Products.csv contains core product information")
            
            if 'suppliers.csv' in available_files['csv_files']:
                suggestions['secondary_files'].append('csv/suppliers.csv')
                suggestions['reasoning'].append("Suppliers.csv provides supply chain context")
        
        # Review-related goals
        if any(keyword in goal_lower for keyword in ['review', 'feedback', 'customer', 'opinion']):
            review_files = [f for f in available_files['markdown_files'] if 'review' in f.lower()]
            suggestions['primary_files'].extend([f'markdown/{f}' for f in review_files[:3]])
            if review_files:
                suggestions['reasoning'].append(f"Found {len(review_files)} review files for customer insights")
        
        # Structure-related goals
        if any(keyword in goal_lower for keyword in ['structure', 'part', 'component', 'assembly']):
            for file in ['parts.csv', 'assemblies.csv']:
                if file in available_files['csv_files']:
                    suggestions['secondary_files'].append(f'csv/{file}')
                    suggestions['reasoning'].append(f"{file} provides structural information")
        
        # Default suggestions if nothing specific found
        if not suggestions['primary_files'] and not suggestions['secondary_files']:
            # Suggest some basic files
            for file in ['products.csv', 'suppliers.csv']:
                if file in available_files['csv_files']:
                    suggestions['primary_files'].append(f'csv/{file}')
            suggestions['reasoning'].append("Basic product and supplier data for general knowledge graph")
        
        return tool_success("file_suggestions", suggestions)
        
    except Exception as e:
        return tool_error(f"Error suggesting files: {str(e)}")

def approve_file_selection(tool_context: ToolContext, selected_files: list) -> Dict[str, Any]:
    """Approve and store the selected files for processing."""
    try:
        ...
        for file_path in selected_files:
            ...
            if validation_result['status'] == 'success':
                ...
                if file_info['valid']:
                    ...
                    return tool_error(f"File validation failed for {file_path}: {file_info['errors']}")
            else:
                return validation_result
        
        # Store approved files in context
        approved_files = {
        ...
        return tool_error(f"Error approving files: {str(e)}")
